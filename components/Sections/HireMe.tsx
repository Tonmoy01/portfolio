      ...
      <div className='relative rounded-2xl border border-border-primary bg-surface-secondary overflow-hidden'>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-12 p-8 md:p-12'>
          <div className='flex flex-col justify-between'>
            <div>
              <h2 className='text-2xl font-medium tracking-tight text-text-heading mb-4'>
                Let&apos;s work together
              </h2>
              <p className='text-text-body leading-relaxed mb-8 text-base'>
                Currently open to new opportunities, hiring, and freelance projects. If
            ...
            <div className='space-y-4'>
              <button
                onClick={copyEmail}
                className='flex items-center gap-3 text-sm text-text-secondary hover:text-text-heading transition-colors p-3 rounded-lg hover:bg-surface-hover border border-transparent hover:border-border-primary w-fit -ml-3 cursor-pointer'
              >
                <Icon
                  icon={emailCopied ? 'lucide:check' : 'lucide:mail'}
                  width={18}
                  className={
                    emailCopied ? 'text-emerald-400' : 'text-text-muted'
                  }
                />
                {emailCopied ? 'Copied!' : 'mdtonmoykhan65@gmail.com'}
              </button>
              <a
                href='https://wa.me/8801777671455'
                target='_blank'
                rel='noopener noreferrer'
                className='flex items-center gap-3 text-sm text-text-secondary hover:text-text-heading transition-colors p-3 rounded-lg hover:bg-surface-hover border border-transparent hover:border-border-primary w-fit -ml-3'
              >
                <Icon
                  icon='lucide:phone'
                  width={18}
